...
# SPDX-FileCopyrightText: (c) 2024 enokseth 
...
find_serialno()
{
  ...
  # Vérifications spécifiques aux fabricants et propriétés courantes
  if compare_nocase "${BUILD_MANUFACTURER?}" 'Lenovo' && _val="$(auto_getprop 'ro.lenovosn2')" && is_valid_serial "${_val?}"; then
    : # Lenovo tablets
  elif _val="$(auto_getprop 'ril.serialnumber')" && is_valid_serial "${_val?}"; then
    : # Samsung phones / tablets (possibly others)
  elif _val="$(auto_getprop 'ro.ril.oem.psno')" && is_valid_serial "${_val?}"; then
    : # Xiaomi phones (possibly others)
  elif _val="$(auto_getprop 'ro.ril.oem.sno')" && is_valid_serial "${_val?}"; then
    : # Xiaomi phones (possibly others)
    ...
    :
  elif _val="$(device_get_file_content "${1:?}" '/proc/cpuinfo' | grep -i 'serial' | cut -d ':' -f '2' | trim_space_on_sides)" && is_valid_serial "${_val?}"; then
    : # /proc/cpuinfo
  elif _val="$(device_get_file_content "${1:?}" '/sys/devices/soc0/serial_number')" && is_valid_serial "${_val?}"; then
    : # /sys/devices/soc0/serial_number
  elif _val="$(device_get_file_content "${1:?}" '/proc/device-tree/serial-number')" && is_valid_serial "${_val?}"; then
    : # /proc/device-tree/serial-number
  elif _val="$(device_get_file_content "${1:?}" '/sys/firmware/devicetree/base/serial-number')" && is_valid_serial "${_val?}"; then
    : # /sys/firmware/devicetree/base/serial-number
  elif _val="$(auto_getprop 'ro.product.serialno')" && is_valid_serial "${_val?}"; then
    : # Common property for product serial
  elif _val="$(auto_getprop 'persist.service.serial')" && is_valid_serial "${_val?}"; then
    : # Persisted serial service
  elif _val="$(device_get_file_content "${1:?}" '/data/serialno')" && is_valid_serial "${_val?}"; then
    : # /data/serialno
  elif _val="$(device_get_file_content "${1:?}" '/efs/FactoryApp/serial_no')" && is_valid_serial "${_val?}"; then
    : # /efs/FactoryApp/serial_no
  elif _val="$(device_get_file_content "${1:?}" '/efs/imei/serial_no')" && is_valid_serial "${_val?}"; then
    : # /efs/imei/serial_no
  elif _val="$(device_get_file_content "${1:?}" '/efs/serial')" && is_valid_serial "${_val?}"; then
    : # /efs/serial
  elif _val="$(device_get_file_content "${1:?}" '/mnt/vendor/persist/serialno')" && is_valid_serial "${_val?}"; then
    : # /mnt/vendor/persist/serialno
  elif _val="$(device_get_file_content "${1:?}" '/proc/boardinfo' | grep -i 'serial' | cut -d ':' -f '2' | trim_space_on_sides)" && is_valid_serial "${_val?}"; then
    : # /proc/boardinfo
  elif _val="$(device_shell "${1:?}" 'getprop vendor.serialno')" && is_valid_serial "${_val?}"; then
    : # Vendor-specific serial number
  elif _val="$(device_shell "${1:?}" 'getprop vendor.boot.serialno')" && is_valid_serial "${_val?}"; then
    : # Vendor boot-specific serial number
  elif _val="$(device_shell "${1:?}" 'getprop vendor.kernel.androidboot.serialno')" && is_valid_serial "${_val?}"; then
    : # Vendor kernel serial number
  elif _val="$(device_shell "${1:?}" 'cat /sys/class/dmi/id/product_serial')" && is_valid_serial "${_val?}"; then
    : # /sys/class/dmi/id/product_serial
  elif _val="$(device_shell "${1:?}" 'cat /sys/class/dmi/id/board_serial')" && is_valid_serial "${_val?}"; then
    : # /sys/class/dmi/id/board_serial
  elif _val="$(device_shell "${1:?}" 'cat /sys/devices/platform/soc0/serial_number')" && is_valid_serial "${_val?}"; then
    : # /sys/devices/platform/soc0/serial_number
  elif _val="$(device_shell "${1:?}" 'cat /proc/sys/kernel/random/boot_id')" && is_valid_serial "${_val?}"; then
    : # /proc/sys/kernel/random/boot_id
  else
    show_error "Unable to find serial number for device: ${1:?}"
    return 1
  fi
...
find_cpu_serialno()
{
  ...
  # Vérification 1 : /proc/cpuinfo pour "serial" (méthode actuelle)
  ...
  # Vérification 2 : /proc/device-tree/serial-number
  if _val="$(device_get_file_content "${1:?}" '/proc/device-tree/serial-number')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 3 : /sys/class/dmi/id/product_serial
  if _val="$(device_get_file_content "${1:?}" '/sys/class/dmi/id/product_serial')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 4 : /sys/class/dmi/id/board_serial
  if _val="$(device_get_file_content "${1:?}" '/sys/class/dmi/id/board_serial')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 5 : /sys/devices/soc0/serial_number
  if _val="$(device_get_file_content "${1:?}" '/sys/devices/soc0/serial_number')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 6 : /sys/firmware/devicetree/base/serial-number
  if _val="$(device_get_file_content "${1:?}" '/sys/firmware/devicetree/base/serial-number')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 7 : /proc/boardinfo
  if _val="$(device_get_file_content "${1:?}" '/proc/boardinfo' | grep -i -F -e "serial" | cut -d ':' -f '2-' -s | trim_space_on_sides)" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 8 : /proc/sys/kernel/random/boot_id (utilisé dans certains systèmes embarqués)
  if _val="$(device_get_file_content "${1:?}" '/proc/sys/kernel/random/boot_id')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 9 : /proc/socinfo
  if _val="$(device_get_file_content "${1:?}" '/proc/socinfo' | grep -i -F -e "serial" | cut -d ':' -f '2-' -s | trim_space_on_sides)" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 10 : ro.serialno (via getprop)
  if _val="$(device_shell "${1:?}" 'getprop ro.serialno')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 11 : sys.serialnumber (via getprop)
  if _val="$(device_shell "${1:?}" 'getprop sys.serialnumber')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 12 : ro.boot.serialno (via getprop)
  if _val="$(device_shell "${1:?}" 'getprop ro.boot.serialno')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 13 : ro.kernel.androidboot.serialno (via getprop)
  if _val="$(device_shell "${1:?}" 'getprop ro.kernel.androidboot.serialno')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 14 : persist.service.serial (via getprop)
  if _val="$(device_shell "${1:?}" 'getprop persist.service.serial')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 15 : /efs/FactoryApp/serial_no
  if _val="$(device_get_file_content "${1:?}" '/efs/FactoryApp/serial_no')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 16 : /efs/imei/serial_no
  if _val="$(device_get_file_content "${1:?}" '/efs/imei/serial_no')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 17 : /efs/serial
  if _val="$(device_get_file_content "${1:?}" '/efs/serial')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 18 : /data/serialno
  if _val="$(device_get_file_content "${1:?}" '/data/serialno')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 19 : /mnt/vendor/persist/serialno
  if _val="$(device_get_file_content "${1:?}" '/mnt/vendor/persist/serialno')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi

  # Vérification 20 : fallback via "adb shell cat"
  if _val="$(device_shell "${1:?}" 'cat /proc/cpuinfo | grep -i serial | cut -d ":" -f 2 | tr -d " \n\r"')" && is_valid_serial "${_val?}"; then
    printf '%s' "${_val?}"
    return 0
  fi
...
get_gsf_id() {
    local _val _my_command _alternative_paths

    # Assurez-vous que le type d'entrée est 'adb'
    if [[ "${INPUT_TYPE}" != "adb" ]]; then
        echo "ERROR: Input type must be 'adb'."
        return 1
    fi

    # Requête SQLite à exécuter
    _my_command='PATH="${PATH:-/sbin}:/sbin:/vendor/bin:/system/sbin:/system/bin:/system/xbin}"; export PATH; readonly my_query="SELECT value FROM main WHERE name = \"android_id\";"'

    # Chemins alternatifs où le fichier pourrait exister
    _alternative_paths=(
        "/data/data/com.google.android.gsf/databases/gservices.db"
        "/data/data/com.google.android.gms/databases/gservices.db"
        "/data/misc/profiles/cur/0/com.google.android.gsf/databases/gservices.db"
        "/data/misc/profiles/cur/0/com.google.android.gms/databases/gservices.db"
        "/mnt/vendor/data/com.google.android.gsf/databases/gservices.db"
        "/mnt/vendor/data/com.google.android.gms/databases/gservices.db"
        "/data/vendor/profiles/com.google.android.gsf/databases/gservices.db"
        "/data/vendor/profiles/com.google.android.gms/databases/gservices.db"
        "/system/priv-app/com.google.android.gsf/databases/gservices.db"
        "/system/priv-app/com.google.android.gms/databases/gservices.db"
        "/vendor/data/com.google.android.gsf/databases/gservices.db"
        "/vendor/data/com.google.android.gms/databases/gservices.db"
        "/mnt/sdcard/android/data/com.google.android.gsf/databases/gservices.db"
        "/mnt/sdcard/android/data/com.google.android.gms/databases/gservices.db"
    )

    # Vérification des chemins
    _val=""
    for path in "${_alternative_paths[@]}"; do
        if adb -s "${1:?}" shell "test -e '${path}'"; then
            _val=$(adb -s "${1:?}" shell "${_my_command} sqlite3 2> /dev/null -line '${path}' \"\${my_query}\"") || _val=""
            if [[ -n "${_val}" ]]; then
                break
            fi
        fi
    ...
    # Vérification alternative si _val est vide
    if [[ -z "${_val}" ]]; then
        # Tentative avec le fichier settings.db
        _my_command='readonly my_query="SELECT value FROM main WHERE name = \"android_id\";"'
        _val=$(adb -s "${1:?}" shell "su -c 'sqlite3 /data/system/users/0/settings.db \"\${my_query}\"'") || _val=""
    fi

    # Vérifications pour LineageOS ou autres systèmes personnalisés
    if [[ -z "${_val}" ]]; then
        _val=$(adb -s "${1:?}" shell "su -c 'cat /data/system/gsf_id.txt'") || _val=""
    fi

    if [[ -z "${_val}" ]]; then
        _val=$(adb -s "${1:?}" shell "su -c 'cat /data/system/gsf/android_id.txt'") || _val=""
    fi

    # Traitement de la valeur trouvée
    if [[ -n "${_val}" ]]; then
        # Si _val est trouvé, nettoyage des espaces inutiles et affichage
        _val=$(echo "${_val}" | grep -m 1 -e 'value' | cut -d '=' -f '2-' -s | tr -d '[:space:]')
        printf '%s\n' "${_val# }"
    else
        # Si aucun GSF ID n'est trouvé, retour d'un message d'erreur
        echo "WARNING: GSF ID not found."
        return 1
    fi
}
...
get_device_color()
{
  ...
  # Huawei, Xiaomi, et autres appareils
  if _val="$(auto_getprop 'ro.config.devicecolor')" && is_valid_color "${_val?}"; then # Huawei (possibly others)
    ...
    :
  elif _val="$(auto_getprop 'ro.product.color')" && is_valid_color "${_val?}"; then # Samsung, Pixel (possibly others)
    :
  elif _val="$(auto_getprop 'ro.color')" && is_valid_color "${_val?}"; then # General Android devices
    :
  elif _val="$(auto_getprop 'ro.build.color')" && is_valid_color "${_val?}"; then # General Android devices (extended)
    :
  elif _val="$(auto_getprop 'persist.sys.devicecolor')" && is_valid_color "${_val?}"; then # Additional generic property for color
    :
  else
    _val=''
  fi
...
get_device_back_color()
{
  ...
  # Huawei (probably others) and generic devices
  if _val="$(auto_getprop 'ro.config.backcolor')" && is_valid_color "${_val?}"; then # Huawei (possibly others)
    :
  elif _val="$(auto_getprop 'vendor.panel.backcolor')" && is_valid_color "${_val?}"; then # Xiaomi (possibly others)
    :
  elif _val="$(auto_getprop 'sys.panel.backcolor')" && is_valid_color "${_val?}"; then # Xiaomi (possibly others)
    :
  elif _val="$(auto_getprop 'ro.product.backcolor')" && is_valid_color "${_val?}"; then # Samsung, Pixel (possibly others)
    :
  elif _val="$(auto_getprop 'ro.build.backcolor')" && is_valid_color "${_val?}"; then # General Android devices (extended)
    :
  elif _val="$(auto_getprop 'persist.sys.devicebackcolor')" && is_valid_color "${_val?}"; then # Additional generic property for back color
    :
  else
    _val=''
  fi
...
get_imei_multi_slot()
{
  ...
  # Fonction: String getDeviceIdForPhone(int phoneId, String callingPackage, optional String callingFeatureId)
  ...
  # Ajout de vérifications supplémentaires pour les IMEI
  ...
  # Modèles supplémentaires et vérifications
  if _prop="$(auto_getprop 'ro.product.device')" && [[ "${_prop}" =~ (SM-G973F|SM-N975F|Pixel_5|Poco_X3|Redmi_Note_8|OnePlus_8|Huawei_P40|Realme_6|Moto_G_Pro|Vivo_Y91C) ]]; then
    # Vérification des IMEI pour les modèles populaires
    echo "Device Model: ${_prop}"
    # Plus de vérifications de l'IMEI ici si nécessaire...
  fi
}
...
get_imei()
{
  ...
  # Ajout de modèles supplémentaires pour la vérification de l'IMEI
  if test -n "${_imei_sv?}"; then
    _val="${_imei_sv:?}"
  elif test "${BUILD_VERSION_SDK:?}" -gt "${ANDROID_14_SDK:?}"; then
    _val=''
  elif test "${BUILD_VERSION_SDK:?}" -ge "${ANDROID_13_SDK:?}"; then
    _val="$(call_phonesubinfo "${1:?}" 7 s16 'com.android.shell')" || _val='' # Android 13
  elif test "${BUILD_VERSION_SDK:?}" -ge "${ANDROID_12_SDK:?}"; then
    _val="$(call_phonesubinfo "${1:?}" 6 s16 'com.android.shell')" || _val='' # Android 12
  elif test "${BUILD_VERSION_SDK:?}" -ge "${ANDROID_11_SDK:?}"; then
    _val="$(call_phonesubinfo "${1:?}" 6 s16 'com.android.shell')" || _val='' # Android 11
  elif test "${BUILD_VERSION_SDK:?}" -ge "${ANDROID_5_1_SDK:?}"; then
  ...
  # Vérification de la validité de l'IMEI SV (IMEI Software Version)
  is_valid_length "${_val?}" 2 2
  display_phonesubinfo_or_warn 'IMEI SV' "${_val?}" "${?}" 'non-sensitive'

  # Vérification des modèles supplémentaires (pour IMEI et IMEI SV)
  if _tmp="$(auto_getprop 'ro.product.device')" && [[ "${_tmp}" =~ (SM-G973F|SM-N975F|Pixel_5|Poco_X3|Redmi_Note_8|OnePlus_8|Huawei_P40|Realme_6|Moto_G_Pro|Vivo_Y91C|SM-G991B|Pixel_6|OnePlus_9|Xiaomi_Mi_11|Realme_GT|Redmi_K40|Oppo_F19|Samsung_A52) ]]; then
    # Vérification de l'IMEI et de l'IMEI SV pour les modèles populaires
    echo "Device Model: ${_tmp}"
    # Implémenter des vérifications supplémentaires spécifiques ici si nécessaire
  fi
}
...
get_iccid()
{
  ...
  if test "${BUILD_VERSION_SDK:?}" -gt "${ANDROID_14_SDK:?}"; then
    ...
    _val="$(call_phonesubinfo "${1:?}" 12 s16 'com.android.shell')" || _val='' # Android 13-14
  elif test "${BUILD_VERSION_SDK:?}" -ge "${ANDROID_11_SDK:?}"; then
    _val="$(call_phonesubinfo "${1:?}" 12 s16 'com.android.shell')" || _val='' # Android 11-12
  elif test "${BUILD_VERSION_SDK:?}" -ge "${ANDROID_9_SDK:?}"; then
    _val="$(call_phonesubinfo "${1:?}" 10 s16 'com.android.shell')" || _val='' # Android 9-10
  elif test "${BUILD_VERSION_SDK:?}" -ge "${ANDROID_5_1_SDK:?}"; then
    _val="$(call_phonesubinfo "${1:?}" 11 s16 'com.android.shell')" || _val='' # Android 5.1-8.1
  elif test "${BUILD_VERSION_SDK:?}" -ge "${ANDROID_5_SDK:?}"; then
    _val="$(call_phonesubinfo "${1:?}" 9)" || _val='' # Android 5.0
  elif test "${BUILD_VERSION_SDK:?}" -ge "${ANDROID_4_3_SDK:?}"; then
    _val="$(call_phonesubinfo "${1:?}" 5)" || _val='' # Android 4.3-4.4W
  else
    _val="$(call_phonesubinfo "${1:?}" 4)" || _val='' # Android 4.2 et versions antérieures
  fi
...
main()
{
  ...
  show_script_name "${SCRIPT_NAME:?} v${SCRIPT_VERSION:?} by enok.seth"
...
if test "${execute_script:?}" = 'true'; then
  if test "${change_title:?}" = 'true'; then set_title "${SCRIPT_NAME:?} v${SCRIPT_VERSION:?} by enok.seth"; fi
